...
// string | null | undefined kabul et (s?.get(...) undefined da dönebilir)
...
export default function PERCPage() {
  // İlk render'da tek sefer değerlendirilsin diye lazy initializer kullandık
  ...
  const allNegative =
    ...
    !st.priorVTE &&
    !st.unilateralLeg &&
    !st.recentSurgeryTrauma;

  const params = {
    age50: st.age50 ? 1 : "",
    hr100: st.hr100 ? 1 : "",
    sao2: st.sao2_95 ? 1 : "",
    hemo: st.hemoptysis ? 1 : "",
    est: st.estrogen ? 1 : "",
    vte: st.priorVTE ? 1 : "",
    leg: st.unilateralLeg ? 1 : "",
    sx: st.recentSurgeryTrauma ? 1 : "",
  };
    ...
    <div className="p-4 md:p-8 max-w-3xl mx-auto space-y-6">
      ...
      <div className="rounded-2xl border p-4 text-sm space-y-2">
        ...
        <label className="flex items-center gap-2">
          <input
            type="checkbox"
            checked={st.unilateralLeg}
            onChange={() => toggle("unilateralLeg")}
          />
        ...
        <label className="flex items-center gap-2">
          <input
            type="checkbox"
            checked={st.recentSurgeryTrauma}
            onChange={() => toggle("recentSurgeryTrauma")}
          />
          Yakın cerrahi/Travma
        </label>
      </div>
      ...
      <div className="rounded-2xl border p-4">
        {allNegative ? (
          <div className="text-sm">
            Düşük klinik şüphe + tüm kriterler negatif → <b>PE dışlanabilir</b> (PERC negatif).
          </div>
        ) : (
          <div className="text-sm">
            En az bir kriter pozitif. <b>PERC pozitif</b>; D-dimer/görüntüleme düşünülür.
          </div>
        )}
      </div>
      ...
      <p className="text-xs text-muted-foreground">
        Eğitim amaçlıdır; kılavuzlarla doğrulayın.
      </p>
    </div>
  );
}

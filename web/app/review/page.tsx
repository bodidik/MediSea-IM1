...
export default function ReviewPage(){
  const [plan, setPlan] = React.useState<"free"|"premium"|"pro">("premium"); // gÃ¶rÃ¼nÃ¼m iÃ§in
  ...
  async function submitQuality(id: string, quality: number){
    try{
      ...
      if (!j.ok) throw new Error(j.error || "GÃ¶nderim hatasÄ±");
  ...
  return (
    <div className="p-4 md:p-8 max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl md:text-3xl font-bold">GÃ¼nlÃ¼k Tekrar</h1>
        <PlanBadge plan={plan} />
      </div>
      ...
      <PremiumCard plan={plan} title="Vadesi Gelen Kartlar" min="premium">
        {!items ? (
          ...
          <div className="text-sm text-muted-foreground">Åu an iÃ§in vadesi gelen kart yok.</div>
        ) : (
          <ul className="space-y-3">
            {items.map((it) => (
              <li key={it.id} className="rounded border p-3">
                <div className="text-sm">
                  <b>{it.section || "-"}</b> Â· {it.type || "-"}
                </div>
                <div className="text-xs text-gray-500">
                  Son tarih: {new Date(it.dueAt).toLocaleString("tr-TR")}
                  {" Â· "}AralÄ±k: {it.interval} gÃ¼n
                  {" Â· "}Ease: {it.ease.toFixed(2)}
                </div>
